...
//
// Uses browser.* APIs (Firefox addon system) instead of chrome.*
// Manifest V2 with browser_specific_settings for Firefox.
//
// Supported commands:
...
// Firefox alarms for keep-alive
browser.alarms.create('poll', { periodInMinutes: 0.02 }); // ~1.2s
...
/**
 * Execute a function in the page context (Firefox MV2).
 * Uses browser.tabs.executeScript with function serialization.
 */
async function execInPage(func, tab, args = []) {
  if (!tab) throw new Error('no active tab');
  // Firefox MV2: executeScript runs in the page context by default
  // Serialize function to string, then inject with args
...
async function execute(cmd) {
  ...
  if (action === 'open_tab') {
    ...
    const tab = await browser.tabs.create({ url: cmd.url, active: cmd.active !== false });
  ...
  if (action === 'navigate') {
    ...
    await browser.tabs.update(tab.id, { url: cmd.url });
